...
${site}          http://digdcn-2937.cisco.com/
...
Verify MAC Address Table
    ...
    : FOR  ${device}  IN  @{devices}
    \  ${output} =      execute command "${cmd}" on device "${device}"
    \  Run keyword And Continue on Failure   Should Contain Any        ${output}   Po100  vPC Peer-Link                        MAC Entry does not match expected interface on ${device}     values=false
...
Verify Website
    ...
    check if the title of the page is "DIGDCN-2937"
    Save Screenshot